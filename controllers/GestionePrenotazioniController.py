...
def dispRapido():
    ...
    if request.method == "POST":
        ...
        connection.close()
        if tamponi_disponibili:
            return render_template('/PazienteView/prenota.html', tamponi_disponibili=tamponi_disponibili, tipo="rapido")
        else:
            msg_error = "Nessuna farmacia corrispondente ai parametri di ricerca. Riprovare"
            return render_template('/PazienteView/disponibilitaRapido.html', msg_error=msg_error)
    return render_template('/PazienteView/disponibilitaRapido.html', msg_error=msg_error)
...
def dispMolecolare():
    ...
    if request.method == "POST":
        ...
        if tamponi_disponibili:
            ...
            msg_error = "Nessuna farmacia corrispondente ai parametri di ricerca. Riprovare"
            return render_template('/PazienteView/disponibilitaMolecolare.html', msg_error=msg_error)
    return render_template('/PazienteView/disponibilitaMolecolare.html', msg_error=msg_error)
...
def prenotaNuovo(ID_farmacia,ID_tampone,Giorno):
    ...
    connection.close()
    return render_template('/PazienteView/prenotaNuovo.html', ID_farmacia=ID_farmacia, ID_tampone=ID_tampone, Giorno=Giorno, Orari=Orari, msg_error=msg_error)
...
def confermaPrenotaNuovo(ID_farmacia,ID_tampone,Giorno):
    if request.method == 'POST':
        ...
        try:
            ...
            msg_error = "Attenzione! Sei già presente nel sistema torna indietro e procedi come Paziente Registrato"
            Orari = connection.execute('SELECT Orario FROM Orari WHERE ID_Tampone = ? AND Giorno = ?',(ID_tampone,Giorno,)).fetchall()
            return render_template('/PazienteView/prenotaNuovo.html', ID_farmacia=ID_farmacia, ID_tampone=ID_tampone, Giorno=Giorno, Orari=Orari, msg_error=msg_error)
